...
func (c *Command) Run(args []string) int {
	...
	response, err := http.Get(fmt.Sprintf("%s/clusters", pf.Endpoint()))
	if err != nil {
		c.UI.Output("Error retrieving clusters:\n%v", err, terminal.WithErrorStyle())
		return 1
	}
	...
	if err != nil {
		c.UI.Output("Error reading clusters:\n%v", err, terminal.WithErrorStyle())
		return 1
	}
