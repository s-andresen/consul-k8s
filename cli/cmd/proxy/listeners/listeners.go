...
func (c *Command) Run(args []string) int {
	...
	response, err := http.Get(fmt.Sprintf("%s/listeners", pf.Endpoint()))
	if err != nil {
		c.UI.Output("Error retrieving listeners:\n%v", err, terminal.WithErrorStyle())
		return 1
	}
	...
	if err != nil {
		c.UI.Output("Error reading listeners:\n%v", err, terminal.WithErrorStyle())
		return 1
	}
